    ...
    return (
        <section id='loginPopup' className='fixed top-0 left-0 w-full h-full bg-black/50 z-[10000] flex items-center justify-center'>
            <div className='w-[90%] md:w-[60%] lg:w-[40%] 2xl:w-[30%] flex flex-col items-center justify-center rounded-[20px] shadow-lg bg-white overflow-hidden'>
                ...
                <form className='w-[80%] flex flex-col items-center justify-center overflow-hidden'>
                    <input className={inputStyle} placeholder='Email' type="email" />
                    <input className={inputStyle} placeholder='Password' type="password" />
                </form>
                
                <Link className='font-inika w-[80%] text-right text-[13px] text-red-500 leading-none md:text-[15px]'>Forgot password?</Link>
                ...
                <p className='w-[70%] text-center font-inika text-[13px] mb-8 md:text-[15px] md:mb-15'>Don’t have an account yet? <Link className='text-red-500'>Sign up</Link> and sip up!</p>
            </div>
        </section>
    )
}
